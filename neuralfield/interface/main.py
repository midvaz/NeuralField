...
# -----------------------------------------------------------------------------
# Main image analysis stub with optional mocking
# -----------------------------------------------------------------------------
...
class FieldAnalyzerApp:
    ...
    #     if not ev.files:
    #         return
    #     save_path = Path(ev.files[0].path)
    #     report = {
    #         'classes': self.output_text.value.splitlines()[0].replace(LANGUAGES[self.lang]['lbl_classes'], '').split(', '),
    #         'recommendations': self.output_text.value.splitlines()[1:],
    #         'gps': None if LANGUAGES[self.lang]['gps_missing'] in self.output_text.value else ev.files[0].path
    #     }
    #     try:
